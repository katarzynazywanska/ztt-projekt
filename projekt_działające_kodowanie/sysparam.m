...
% System params  
    ...
    ip.addParameter('psize', 2, @isnumeric);
    ip.addParameter('M', 2, @isnumeric);
    ip.addParameter('snr', 10, @isnumeric);
    ip.addParameter('packet_lenght', '[500 5000 50000]', @ischar);
    ip.addParameter('ofdmsize', 64, @isnumeric);
    ip.addParameter('ofdmdatasubsize', 52, @isnumeric);
    ip.addParameter('guardsamples', 10, @isnumeric);
    ip.addParameter('crcsize', 32, @isnumeric);
    ip.addParameter('fftsize', 1024, @isnumeric);
    ip.addParameter('OFDM', true, @islogical);
    ip.addParameter('CRCpoly', '0x04C11DB7', @ischar);
    ip.addParameter('scater', '.', @ischar);
    ip.addParameter('oversampling', 16, @isnumeric);
    ip.addParameter('ofdm_delay', 0, @isnumeric);
    ip.addParameter('fig', 0, @isnumeric);
    ip.parse( varargin{:} );
    r = ip.Results;
...
% System params
    ...
    % demodulation matrix
    sysparams.constm = repmat(sysparams.constelation, 4*sysparams.symbolsno, 1);
end
